...
`gstack` 只决定“怎么分阶段想”，不决定“要做什么产品方向”。

- `reframe`
  - 吸收 `office-hours` 风格的问题澄清与约束识别。
  - 用于把模糊任务压缩成单一目标、成功标准、非目标和关键假设。
- `plan`
  - 吸收 `plan-ceo-review` 的价值、范围、优先级检查视角。
  - 吸收 `plan-eng-review` 的技术路径、依赖、风险、复杂度检查视角。
- `review`
  - 吸收 `review` 的对抗式工程审查姿态。
- `verify`
  - 吸收 `qa` 的验证视角，但只保留“把验收点映射到证据”的部分。
- `retro`
  - 吸收 `retro` 的流程学习姿态。
...
`superpowers` 是手脚，不是方向盘。它只在“阶段已经过 gate”且“ticket 已经激活”的范围内提供受控执行能力。
